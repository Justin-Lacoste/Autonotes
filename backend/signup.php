...
$sql = ("INSERT INTO autonotes_users (email, password, salt, token) VALUES (?, ?, ?, ?)");

$stmt = $db->prepare($sql); 
$stmt->bind_param("ssss", $email, $encryptedPassword, $salt, $token);
$stmt->execute();
$last_id = $stmt->insert_id;

if (!empty($last_id)) {

	$entityBody = '{"TableName": "autonotesone", "Item": {"users": {"S": "' . $email . '"}, "id": {"N": "' . $last_id . '"}, "password": {"S": "' . $encryptedPassword . '"}, "salt": {"S": "' . $readable_salt . '"}, "token": {"S": "' . $token . '"}, "clock": {"S": "3600"}, "date_created": {"S": "' . $date . '"}}}';
...
$request_headers['x-amz-target'] = "--.PutItem";
